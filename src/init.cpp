...
void Shutdown()
{
    ...
    RenameThread("BitcoinFinal-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: BitcoinFinal.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: BitcoinFinald.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeBitcoinFinalamount=<n> " + _("Keep N BitcoinFinal anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. BitcoinFinal is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitcoinFinal is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("BitcoinFinal version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "BitcoinFinal server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitcoinFinal") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart BitcoinFinal to complete") << "\n";
    ...
    nAnonymizeBitcoinFinalAmount = GetArg("-anonymizeBitcoinFinalamount", 0);
    if(nAnonymizeBitcoinFinalAmount > 999999) nAnonymizeBitcoinFinalAmount = 999999;
    if(nAnonymizeBitcoinFinalAmount < 2) nAnonymizeBitcoinFinalAmount = 2;
    ...
    LogPrintf("Anonymize BitcoinFinal Amount %d\n", nAnonymizeBitcoinFinalAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BTCF+1000 == (.1BTCF+100)*10
       10BTCF+10000 == (1BTCF+1000)*10
    */
