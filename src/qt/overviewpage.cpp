...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitcoinFinalAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitcoinFinalAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitcoinFinalAmount = strAnonymizeBitcoinFinalAmount.remove(strAnonymizeBitcoinFinalAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitcoinFinalAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitcoinFinalAmount*COIN) nMaxToAnonymize = nAnonymizeBitcoinFinalAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitcoinFinalAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitcoinFinalAmount));
        strAnonymizeBitcoinFinalAmount = strAnonymizeBitcoinFinalAmount.remove(strAnonymizeBitcoinFinalAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitcoinFinalAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeBitcoinFinalAmount == 0){
