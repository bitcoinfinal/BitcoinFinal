...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("BitcoinFinal"))
        return false;
...
{
    // Convert BitcoinFinal:// to BitcoinFinal:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("BitcoinFinal://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "BitcoinFinal:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinFinal.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinFinal.lnk
...
{
    return GetAutostartDir() / "BitcoinFinal.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a BitcoinFinal.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BitcoinFinal\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the BitcoinFinal app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add BitcoinFinal app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitcoinFinal-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BitcoinFinal-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BitcoinFinal-Qt"));
