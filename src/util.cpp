...
int nAnonymizeBitcoinFinalAmount = 1000;
...
#else
    const char* pszModule = "BitcoinFinal";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinFinal
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinFinal
    // Mac: ~/Library/Application Support/BitcoinFinal
    // Unix: ~/.BitcoinFinal
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinFinal";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BitcoinFinal.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BitcoinFinald.pid"));
